...
export function FaqSection() {
  ...
  function addItem() {
    setItems(prev => [...prev, {
      id: crypto.randomUUID(), question: '', answer: '', sort_order: prev.length,
    }])
  }
  ...
  async function removeItem(item: FaqItem, i: number) {
    setItems(prev => prev.filter((_, idx) => idx !== i))
    await deleteChildItem('faq_items', item.id).catch(() => null)
  }
  ...
  async function save() {
    ...
    try {
      await Promise.all(
        items.map((item, i) => upsertChildItem('faq_items', { ...item, sort_order: i } as Record<string, unknown>))
      )
  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-3">
        {items.map((item, i) => (
          <div key={item.id} className="flex items-start gap-3 p-4 rounded-xl"
            ...
            <div className="flex-1 flex flex-col gap-3">
              <TextField label="Question" value={item.question}
                onChange={e => updateItem(i, { question: e.target.value })} />
              <TextareaField label="Answer" rows={4} value={item.answer}
        ...
        {items.length === 0 && (
          <p className="text-sm" style={{ color: 'var(--ci-muted)' }}>No FAQ items yet — click Add Question to create one.</p>
        )}
      </div>
    </section>
  )
