...
const statusConfig: Record<Status, { label: string; color: string; bg: string }> = {
  draft: { label: 'Draft', color: 'var(--ci-muted)', bg: '#f0f0f0' },
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-8 py-4 flex-shrink-0"
        ...
        <h1 className="text-lg font-semibold" style={{ color: 'var(--ci-navy)' }}>Articles</h1>
      ...
      <div className="flex items-center gap-0 px-8"
        ...
        {(['all', 'published', 'draft', 'scheduled'] as Filter[]).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={{ color: filter === f ? 'var(--ci-navy)' : 'var(--ci-muted)', border: 'none', background: 'transparent', cursor: 'pointer' }}>
            {f} <span className="ml-1 text-xs" style={{ color: 'var(--ci-muted)' }}>({counts[f]})</span>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            <FileText size={40} style={{ color: 'var(--ci-border)' }} />
              ...
              <tr style={{ borderBottom: '1px solid var(--ci-border)' }}>
                {['Title', 'Category', 'Status', 'Date', ''].map(h => (
                  <th key={h} className="text-left px-8 py-3 text-xs font-semibold uppercase tracking-wide"
                    style={{ color: 'var(--ci-muted)', background: 'var(--ci-linen)' }}>
                    {h}
                  </th>
                ))}
              </tr>
